...
`POST /api/datasources/import-screenshot` 会保存截图原图，并可接收 `recognizedText`。传入 `recognizedText` 时，系统会把这段 OCR 或手动校正文字解析成题材趋势和作品表现数据；只传截图时，系统会在配置了 OpenAI Key 的情况下通过 Responses API 读取图片文字，再进入同一套解析流程。没有 Key、识别失败或图片里没有可用文字时，则登记为等待校正的采集任务。
...
`PATCH /api/settings` 可保存 AI 供应商、写作模型、Embedding 模型和 OpenAI API Key。Key 只写入本机 `.env.local` 并更新当前运行状态，接口响应只返回“已配置/未配置”，不会回传 Key 原文；传入 `clearOpenAiApiKey` 可清除本机保存的 Key。
